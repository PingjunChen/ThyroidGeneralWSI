...
def load_gt_prob(json_path, cat=0):
    with open(json_path) as fp:
        data_dict = json.load(fp)
        gt = np.asarray(data_dict["gts"])[:, cat]
        prob = np.asarray(data_dict["preds"])[:, cat]
    return gt, prob



def draw_roc(pred_dict, inset=False):
    #Plot all roc curve
    fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(12, 5))
    with PdfPages('thyroid_benign_roc.pdf') as pdf:
        for i, (k, v) in enumerate(pred_dict.items()):
            gt_list, prob_list = v["gts"], v["probs"]
            split_num = len(gt_list)
            axes[i].plot([0, 1], [0, 1], 'k--')
            if inset == True:
                axins = zoomed_inset_axes(axes[i], 3, loc="center")
            auc_list = []
            ...
            axes[i].set_title(k)
            if inset == True:
                x1, x2, y1, y2 = 0.0, 0.1, 0.8, 1.0 # specify the limits
